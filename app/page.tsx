...
export default function Home(){
  ...
  return <main>
    ...
    <section className="workspace">
      ...
      <section className="grid">{visible.map((company,i)=>{const row=rows[key(day,company)]||blank(day,company);const touched=!!(row.student_number||row.feedback||row.pic_1||row.pic_2);return <article key={company} onClick={()=>setEditing(row)} tabIndex={0} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setEditing(row)}}}>
        <div className="card-top"><span className="number">{String(i+1).padStart(2,"0")}</span><span className={`state ${row.resume_collected?"complete":touched?"progress":""}`}><i/>{row.resume_collected?"Resume collected":touched?"In progress":"Not started"}</span></div>
        <h3>{company}</h3><div className="card-meta"><span><small>STUDENTS</small><b>{row.student_number||"—"}</b></span><span><small>OWNER</small><b>{row.pic_1||row.pic_2||"Unassigned"}</b></span>{(row.chat_time_1||row.chat_time_2)&&<span><small>CHAT</small><b>{row.chat_time_2||row.chat_time_1}</b></span>}</div><button className="edit" type="button">Open record <span>↗</span></button>
      </article>})}</section>
    ...
    {editing&&<div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&setEditing(null)}><form className="drawer" onSubmit={e=>{e.preventDefault();save(editing)}}>
      ...
      <div className="form-grid">
        ...
        <div className="round-label full"><span>01</span><b>First conversation</b></div><label><span>Chat time</span><input type="time" value={editing.chat_time_1} onChange={e=>setEditing({...editing,chat_time_1:e.target.value})}/></label><label><span>PIC / owner</span><input value={editing.pic_1} onChange={e=>setEditing({...editing,pic_1:e.target.value})} placeholder="Team member name"/></label>
        <div className="round-label full"><span>02</span><b>Follow-up conversation</b></div><label><span>Chat time</span><input type="time" value={editing.chat_time_2} onChange={e=>setEditing({...editing,chat_time_2:e.target.value})}/></label><label><span>PIC / owner</span><input value={editing.pic_2} onChange={e=>setEditing({...editing,pic_2:e.target.value})} placeholder="Team member name"/></label>
